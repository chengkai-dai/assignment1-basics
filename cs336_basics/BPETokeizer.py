...
def train_bpe(
    ...
    **kwargs,
    ...
    print("pretokens length:", len(pretokens))
    ...
    if o_index is not None and u_index is not None:
        print(f"Count for ('o','u'): {counts.get((o_index, u_index), 0)}")
    
    if i_index is not None and t_index is not None:
        print(f"Count for ('i','t'): {counts.get((i_index, t_index), 0)}")
    ...
    for i in range(num_merges):
        ...
        print(f"Max pair: {vocab[max_pair[0]]} + {vocab[max_pair[1]]} (count: {counts[max_pair]})")
